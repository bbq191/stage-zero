...
pub async fn post_new_course_db(pool: &PgPool, new_course: CreateCourse) -> Result<Course, MyError> {
    let row = sqlx::query_as!(Course,
        r#"insert into courses(teacher_id,name,description,format,structure,duration,language,price,level) 
        values($1,$2,$3,$4,$5,$6,$7,$8,$9)
        returning id,teacher_id, name,time,description,format,structure,duration,language,price,level"#,
...
pub async fn update_course_details_db(
    ...
    let current_course_row = sqlx::query_as!(
        Course,
        "select * from courses where teacher_id = $1 and id=$2",
        teacher_id,
        course_id
    )
    .fetch_one(pool)
    ...
    let course_row = sqlx::query_as!(Course, 
        "update courses 
set name=$1, description=$2, format=$3,structure=$4,duration=$5,price=$6,language=$7,level=$8 where teacher_id=$9 and id=$10
returning id, teacher_id, name, time, description,format,structure,duration,price,language,level",
    name,description,format,structure,duration,price,language,level,teacher_id,course_id, ).fetch_one(pool).await;
